...
export default function DashboardLayout({
  ...
  const openNotifications = () => {
    toast.info("Funcionalidad pendiente", {
      description: "El panel de notificaciones aún está en desarrollo.",
    })
  }
  ...
  const handleLogout = async () => {
    try {
      ...
      if (typeof document !== "undefined") {
        document.cookie =
          "aeg_role=; Path=/; Max-Age=0; SameSite=Lax"
        document.cookie =
          "aeg_head=; Path=/; Max-Age=0; SameSite=Lax"
      }
        ...
        <div className="w-full relative flex items-center">
          <div className="flex-shrink-0 flex items-center gap-2">
            <button
              onClick={() => router.push("/")}
              aria-label="home"
              className="p-0 bg-transparent border-0 flex items-center"
            >
              <Image
                src="/logo.jpg"
                alt="Logo"
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button className="flex items-center gap-2 rounded-full px-3 py-1 bg-muted text-muted-foreground hover:bg-muted/80">
                  <span className="w-8 h-8 rounded-full bg-primary inline-block" />
                  <span className="text-sm">
                    {user.name || user.account || "Usuario"}
                  </span>
                </button>
              </PopoverTrigger>
              ...
              <PopoverContent className="w-80">
                <div className="space-y-4">
                  <h4 className="font-medium">Detalles de Perfil</h4>
