...
            : undefined // Usa undefined si no hay score (HEAD)
        )
      } catch (err) {
        console.error("Error loading grading data:", err)
        toast.error("Error al cargar los datos del examen.") // Línea 8abe452
      } finally {
        setLoading(false)
      }
    }
...
      
      router.back() // La redirección se hace dentro de la promesa, pero esta línea estaba en 8abe452 antes del toast.
    }

    // Usamos toast.promise para manejar la operación de guardado
    toast.promise(saveOperation(), {
      loading: 'Guardando calificaciones...',
      success: () => {
        setSaving(false)
        // router.back() se movió dentro de saveOperation para asegurar la ejecución
        return "Calificación guardada correctamente.";
      },
      error: (err) => {
        console.error("Error saving grading:", err)
        setSaving(false)
        return err?.message || "Error al guardar la calificación. Inténtalo de nuevo.";
      },
    });
  }
...
          <p className="text-sm text-muted-foreground">Estudiante: {examStudent?.student_name ?? studentId}</p>
        </div>
...
            {/* NUEVO: Respuesta Esperada de 8abe452 */}
            {question.answer && (
              <div>
