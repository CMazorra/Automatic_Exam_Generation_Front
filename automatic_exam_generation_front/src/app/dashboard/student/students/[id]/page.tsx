  ...
  useEffect(() => {
    const loadTeacherSubjects = async () => {
      try {
        ...
        if (myId) {
          const ts = await getSubjectsByStudentID(String(myId))
          setTeacherSubjects(ts || [])
        } else {
          setTeacherSubjects([])
        }
      } catch (e) {
        console.error("Error fetching current user subjects:", e)
        setTeacherSubjects([])
      } finally {
    ...
    <main className="min-h-screen bg-background p-6">
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="space-y-6 rounded-xl border bg-card p-6 shadow-sm">
          ...
          <div className="grid gap-4">
            <div>
              <div className="text-sm text-muted-foreground">Asignaturas en común</div>
              <div className="mt-1">
                {isLoadingSubjects || isLoadingTeacherSubjects ? (
                  ...
                  <p className="mt-1 whitespace-pre-wrap">El alumno logeado no comparte ninguna de las asignaturas de este estudiante.</p>
                ) : (
